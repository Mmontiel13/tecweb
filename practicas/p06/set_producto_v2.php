<?php
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($nombre) || empty($marca) || empty($modelo) || empty($detalles) || empty($imagen) || empty($precio) || empty($unidades)){
            echo 'Error: Uno o mas campos estan vacios.';   
        } else{
            if(!is_numeric($unidades) || floatval($precio) != $precio){
                echo 'Error: Precio debe ser un decimal valido.';
            }else{
                if(!ctype_digit($unidades)){
                    echo 'Error: las unidades deben ser un numero entero.';
                }else{
